...
class Service(ComponentManager):
    ...
    def __init__(
        ...
        Args:
            ...
            state (int, optional): Service state (0 for stateless services).
              Defaults to 0.
        """
    ...
    def _to_dict(self) -> dict:
        ...
        dictionary = {
            ...
            "relationships": {
                "application": {
                    "class": type(self.application).__name__,
    ...
    def collect(self) -> dict:
        ...
        if len(self._migrations) > 0:
            ...
            last_migration = {
                "status": self._migrations[-1]["status"],
                "origin": str(self._migrations[-1]["origin"]),
                "target": str(self._migrations[-1]["target"]),
                "start": self._migrations[-1]["start"],
                "end": self._migrations[-1]["end"],
                "waiting": self._migrations[-1]["waiting_time"],
                "pulling": self._migrations[-1]["pulling_layers_time"],
                "migr_state": self._migrations[-1]["migrating_service_state_time"],
            }
        else:
            last_migration = None
    ...
    def step(self):
        ...
        if len(self._migrations) > 0 and self._migrations[-1]["end"] is None:
            migration = self._migrations[-1]

            # Gathering information about the service's image
            image: ContainerImage = ContainerImage.find_by(
                attribute_name="digest", attribute_value=self.image_digest
            )
            ...
            layers_downloaded = [
                layer
                for layer in migration["target"].container_layers
            ...
            # Setting the migration status to "pulling_layers" once any of the service
            # layers start being downloaded
            ...
            if migration["status"] == "pulling_layers" and len(
                image.layers_digests
            ) == len(layers_downloaded):
                # Once all the layers that compose the service's image are pulled, the
                # service container is deprovisioned on its origin host even though it
                # still is in there (that's why it is still on the origin's services
                # list). This action is only taken in case the current provisioning
                # process regards a migration.
                if self.server:
                    self.server.cpu_demand -= self.cpu_demand
                    self.server.memory_demand -= self.memory_demand

                # Once all service layers have been pulled, creates a ContainerImage
                # object representing the service image on the target host if that host
                # didn't already have such image
                if not any(
                    [
                        image.digest == self.image_digest
                        for image in migration["target"].container_images
                    ]
                ):
                    # Finding similar image provisioned on the infrastructure to get
                    # metadata from it when creating the new image
                    template_image = ContainerImage.find_by(
                        attribute_name="digest", attribute_value=self.image_digest
                    )
                    if template_image is None:
                        raise Exception(
                            "Could not find any container image with digest:"
                            f"{self.image_digest}"
                        )
                ...
                if self.state == 0 or self.server is None:
                    # Stateless Services: migration is set to finished immediately
                    # after layers are pulled
                    migration["status"] = "finished"
                else:
                    # Stateful Services: state must be migrated to the target host
                    # after layers are pulled
                    migration["status"] = "migrating_service_state"

                    # Services are unavailable during the period where their states are
                    # being migrated
                    ...
                    # Creating network flow representing the service state that will be
                    # migrated to its target host
                    flow = NetworkFlow(
            ...
            if migration["status"] == "finished":
                ...
                # Changing the routes used to communicate the application that owns the
    ...
    def provision(self, target_server: object):
        ...
        being hosted by a server and we want to relocate it to another server within
        the infrastructure.
        ...
        # Gathering the list of layers that compose the service image that are not
        # present in the target server
        image = ContainerImage.find_by(
            attribute_name="digest", attribute_value=self.image_digest
        )
        for layer_digest in image.layers_digests:
            if not any(
                ...
                # As the image only stores its layers digests, we need to get
                # information about each of its layers
                layer_metadata = ContainerLayer.find_by(
                ...
                # Adding the layer to the target server's waiting queue (layers it must
        ...
        # Telling EdgeSimPy the service's current server is now performing a migration.
        # This action is only triggered in case this method is called for performing a
        # migration (i.e., the service is already within the infrastructure)
        if self.server:
            self.server.ongoing_migrations += 1

        # Reserving the service demand inside the target server and telling EdgeSimPy
        # that server will receive a service
